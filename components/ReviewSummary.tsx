...
import { ChevronLeft, CheckCircle2, Download, Loader2, Send, Edit3, BookOpen, Info, Mail } from 'lucide-react';
...
const ReviewSummary: React.FC<Props> = ({ data, lang, onBack, onSubmit, onComplete, user }) => {
  ...
  const handleEmailHomework = () => {
    if (!lesson) return;
    const subject = `BSF Homework: Lesson ${lesson.id} - ${user?.name || ''}`;
    const body = `Hi, attached is my scripture analysis for Lesson ${lesson.id}.\n\n(Please attach the PDF before sending)`;
    ...
    return (
      <div id="review-export-template" className="hidden-export-container p-12 bg-white text-gray-900 font-sans leading-relaxed" style={{ width: '800px', margin: '0 auto' }}>
        <div className="border-b-2 border-gray-900 pb-6 mb-10 space-y-2">
          ...
          <div className="flex gap-2"><span className="font-bold min-w-[120px]">{t.pdfUser}:</span><span>{user?.name || "Guest"} ({user?.email || ""})</span></div>
        </div>
        <section className="mb-10">
          <h3 className="font-bold border-b border-gray-400 mb-4 pb-1 tracking-wider uppercase">{HEADERS.doctrine}</h3>
          <div className="pl-4"><p className="font-bold text-lg">{isZh ? lesson?.doctrineZh : lesson?.doctrineEn}</p></div>
        </section>
          ...
          <div className="pl-4 space-y-4">
            {data.step1.map((item) => (
              <div key={item.id} className="space-y-1">
                ...
                <div className="flex gap-3 pl-4"><span className="shrink-0">-</span><p>{item.text}</p></div>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="pl-4 space-y-4">
            {data.step2.map((s, idx) => (
              <div key={s.id} className="flex gap-3">
                ...
                <div>
                  <span className="text-sm font-bold block mb-1">{isZh ? `应用问题#${idx + 1}` : `Application ${idx + 1}`}</span>
                  <p>{data.step5[s.id]}</p>
                </div>
              </div>
            ))}
    ...
    <div className="max-w-5xl mx-auto pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={handleEmailHomework} className="flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-indigo-100 text-indigo-600 font-bold rounded-2xl hover:bg-indigo-50 transition-all text-[14px]">
             <Mail size={16} /> {isZh ? "邮件作业" : "Email PDF"}
          </button>
      ...
      <div className="bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden mb-12">
        <div className="px-10 py-10 border-b bg-gray-50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-[12px] font-black text-blue-500 uppercase tracking-[0.3em] mb-2">{isZh ? `第 ${data.lessonId} 课` : `Lesson ${data.lessonId}`}</h2>
              <h1 className="text-4xl font-black text-[#1A2B3C] tracking-tight">{isZh ? lesson?.titleZh : lesson?.titleEn}</h1>
              <div className="mt-4 text-sm font-bold text-gray-400 uppercase tracking-widest">{bookLabel}{rangeLabel}</div>
            </div>
        ...
        <div className="p-10 space-y-16">
          {lesson?.highlightedVerseZh && (
             <section className="p-8 bg-amber-50/50 rounded-[32px] border border-amber-100/50">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-2 h-6 bg-amber-500 rounded-full" /><h4 className="text-[14px] uppercase font-black text-[#1A2B3C] tracking-[0.2em]">{HEADERS.highlighted}</h4>
              </div>
              <div className="space-y-4">
                 <p className="text-[20px] text-[#1A2B3C] font-bold leading-relaxed italic">"{isZh ? lesson.highlightedVerseZh : (highlightedVerseEn || "Loading English version...")}"</p>
                 <div className="text-[12px] font-black text-amber-600 uppercase tracking-widest bg-white inline-block px-4 py-1.5 rounded-full border border-amber-100 shadow-sm">{getPassageText(lesson.highlightedPassage)}</div>
              </div>
            </section>
          )}
          <section>
            <div className="flex items-center gap-3 mb-6"><div className="w-2 h-6 bg-blue-500 rounded-full" /><h4 className="text-[14px] uppercase font-black text-[#1A2B3C] tracking-[0.2em]">{HEADERS.content}</h4></div>
          ...
          <section className="p-8 bg-[#F0F7FF] rounded-[32px] border border-blue-100/50">
            <div className="flex items-center gap-3 mb-6"><div className="w-2 h-6 bg-blue-500 rounded-full" /><h4 className="text-[14px] uppercase font-black text-[#1A2B3C] tracking-[0.2em]">{HEADERS.division}</h4></div>
            <div className="space-y-4">
              {data.step2.map((s, i) => (
                <div key={s.id} className="p-6 bg-white rounded-2xl border border-blue-100/30 shadow-sm">
                  <p className="text-[18px] text-gray-800 font-bold leading-relaxed"><span className="mr-2 text-blue-500">{toRoman(i + 1)}.</span>{s.summary}<span className="ml-2 text-[14px] font-normal text-gray-400 italic">{getFullDivisionRef(s)}</span></p>
                </div>
              ))}
            </div>
          </section>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section className="p-8 bg-[#FFF9EB] rounded-[32px] border border-amber-100/50">
              <div className="flex items-center gap-3 mb-4"><div className="w-2 h-6 bg-amber-500 rounded-full" /><h4 className="text-[14px] uppercase font-black text-[#1A2B3C] tracking-[0.2em]">{HEADERS.subject}</h4></div>
              <div className="p-6 bg-white/80 rounded-2xl border border-amber-100/30 italic text-[19px] text-[#1A2B3C] font-medium leading-relaxed">"{data.step3}"</div>
            </section>
            <section className="p-8 bg-[#F0FFF4] rounded-[32px] border border-green-100/50">
              <div className="flex items-center gap-3 mb-4"><div className="w-2 h-6 bg-green-500 rounded-full" /><h4 className="text-[14px] uppercase font-black text-[#1A2B3C] tracking-[0.2em]">{HEADERS.aim}</h4></div>
              <div className="p-6 bg-white/80 rounded-2xl border border-green-100/30 text-[19px] text-gray-800 leading-relaxed font-medium">{data.step4}</div>
            </section>
          </div>
          <section className="p-8 bg-[#F5F3FF] rounded-[32px] border border-indigo-100/50">
            <div className="flex items-center gap-3 mb-6"><div className="w-2 h-6 bg-indigo-500 rounded-full" /><h4 className="text-[14px] uppercase font-black text-[#1A2B3C] tracking-[0.2em]">{HEADERS.application}</h4></div>
            <div className="space-y-5">
              {data.step2.map((s, i) => (
                <div key={s.id} className="p-6 bg-white border border-indigo-100/30 rounded-2xl flex gap-6 shadow-sm">
                  <div className="shrink-0"><span className="w-10 h-10 rounded-2xl bg-[#1A2B3C] text-white flex items-center justify-center font-black text-[18px] shadow-lg">{i + 1}</span></div>
                  <div className="space-y-2">
                    <div className="text-[11px] font-black text-indigo-400 uppercase tracking-widest">{isZh ? `应用问题#${i + 1}` : `Application ${i + 1}`}</div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-50">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <button onClick={onBack} className="flex items-center gap-2 px-6 py-4 text-gray-500 font-black hover:bg-gray-50 rounded-2xl transition-all"><Edit3 size={18} />{isZh ? "返回修改" : "Back to Edit"}</button>
          <div className="flex gap-4">
            ...
            {isSubmitted ? (
              <div className="flex items-center gap-2 px-10 py-4 bg-green-50 text-green-600 rounded-full border border-green-100 font-black text-[14px]"><CheckCircle2 size={18} />{isZh ? "作业已提交" : "Completed"}</div>
            ) : (
              <button onClick={handleSubmit} disabled={isSubmitting} className="flex items-center gap-2 px-10 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-lg hover:bg-blue-700 transition-all">
                {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <Send size={18} />}{t.submit}
              </button>
            )}
          </div>
        </div>
      </div>
      <div className="fixed top-0 left-0 w-0 h-0 overflow-hidden pointer-events-none">{renderSimplifiedExportTemplate()}</div>
    </div>
  );
};
